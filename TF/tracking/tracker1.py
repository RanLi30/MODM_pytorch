...
class Tracker():
    ...
    def estimate_bbox(self, responses, x_roi_size_origs, target_pos, target_size,radius,center,frame_idx):
        ...
        if frame_idx > 1:
            ...
            mask = gen_crater_mask(shape, mask_center,config.radiusscale*radius, 5*config.sigma)
            ...
            masked_response_shr=(mask_shr*response_shr)
            
            response = cv2.resize(masked_response_shr,(shape[0], shape[1]),interpolation=cv2.INTER_CUBIC) 
