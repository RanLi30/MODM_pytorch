...
import sys
import config
from model2 import build_initial_state, build_model, ModeKeys
...
class Model():
    def __init__(self, sess, checkpoint_dir=None):
        ...
        self.response, saver, self.final_state = build_model(z_exemplar, self.x_instances, mem_cell, self.initial_state, ModeKeys.PREDICT,self.cur_frame_idx,self.center,self.redius)
...
class Tracker():
    ...
    def __init__(self, model):
        ...
        scale_steps_x = list(range(math.ceil(config.num_scale / 2) - config.num_scale, math.floor(config.num_scale / 2) + 1))
        scale_steps_y= list(range(math.ceil(config.num_scale / 2) - config.num_scale, math.floor(config.num_scale / 2) + 1))
    ...
    def estimate_bbox(self, responses, x_roi_size_origs, target_pos, target_size,radius,center,frame_idx):
        ...
        up_response_size = config.response_size * config.response_up
        current_scale_idx = math.floor(config.num_scale*config.num_scale / 2)#middle positon
        ...
        #############################
        if frame_idx > 1:
            response = gen_crater_mask(response, center,5* radius, config.sigma)
        #############################
    ...
    def track(self, cur_frame_file, redius, center, cur_frame_idx,display=False ):
        ...
        if display:
            ...
            return bbox, cur_frame,responses[best_scale_idx],x_instances[best_scale_idx, 0]
