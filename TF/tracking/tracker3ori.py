...
class Model():
    def __init__(self, sess, checkpoint_dir=None):
        ...
        self.x_roi = tf.placeholder(tf.float32, [config.num_scale*config.num_scale, 4], name='x_roi')
        ...
        init_z_exemplar = tf.tile(init_z_exemplar, [config.num_scale*config.num_scale, 1, 1, 1, 1])
        ...
        z_exemplar = tf.tile(z_exemplar, [config.num_scale*config.num_scale, 1, 1, 1, 1])
        ...
        self.x_instances = tf.reshape(self.x_instances, [config.num_scale*config.num_scale, 1, config.x_instance_size, config.x_instance_size, 3])
...
class Tracker():
    ...
    def estimate_bbox(self, responses, x_roi_size_origs, target_pos, target_size,radius,center,frame_idx):
        ...
        up_response_size = config.response_size * config.response_up
        current_scale_idx = math.floor(config.num_scale*config.num_scale / 2)
        ...
        for s_idx in range(config.num_scale*config.num_scale):
        ...
        #shift_center_orig = shift_center_instance *np.expand_dims(x_roi_size_orig, 0) / x_roi_size_origs[current_scale_idx][0]
        shift_center_orig = shift_center_instance * bestscaling
        target_pos_new = np.mean(target_pos + shift_center_orig, 0)
        #target_pos_new = np.mean(target_pos + shift_center_instance, 0)
    ...
    def track(self, cur_frame_file, redius, center, cur_frame_idx,display=False ):
        ...
        target_poses = np.tile(np.expand_dims(self.target_pos,axis=0), [config.num_scale*config.num_scale,1])
        x_rois = np.concatenate([target_poses, sx_roi_size], axis=1)
        z_roi = np.concatenate([self.target_pos, self.z_roi_size], 0)
        att_score, responses, cur_frame, x_instances, self.next_state = self._sess.run([self._model.att_score,
        ...
        if display:
            return bbox,self.target_pos, cur_frame, x_instances[best_scale_idx, 0], att_score[best_scale_idx], responses[best_scale_idx], self.next_state.access_state
        else:
            return bbox, self.target_pos,cur_frame,responses[best_scale_idx],x_instances[best_scale_idx, 0]
...
def get_new_state(state, best_scale):
    ...
    c = np.array([c_best]*config.num_scale*config.num_scale)
    h = np.array([h_best]*config.num_scale*config.num_scale)
    ...
    for s in access_state:
        s_best = s[best_scale]
        s_list.append([s_best]*config.num_scale*config.num_scale)
