...
def attention(inpu, query, scope=None):

    input_shape = inpu.get_shape().as_list()
    input_transform = tf.layers.conv2d(inpu, input_shape[-1], [1, 1], [1, 1], use_bias=False, name='input_layer')
    ...
    return tf.reduce_sum(inpu*score, [1,2]), score
...
class MemNet(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, hidden_size, memory_size, slot_size, is_train):
        ...
        self._controller = tf.contrib.rnn.LayerNormBasicLSTMCell(hidden_size, layer_norm=True, dropout_keep_prob=keep_prob)  # this is the LSTM
        
        self._memory_access = access.MemoryAccess(memory_size, slot_size, is_train)
    ...
    def __call__(self, inputs, prev_state, scope=None):
        ...
        controller_input = access.get_key_feature(search_feature, 'search_key')

        attention_input, self.att_score = attention(controller_input, prev_state.controller_state[1], scope)

        controller_output, controller_state = self._controller(attention_input, prev_state.controller_state, scope)

        access_inputs = (memory_for_writing, controller_output)
        access_output, access_state = self._memory_access(access_inputs, prev_state.access_state, scope)
    ...
    def initial_state(self, init_feature):
        ...
        init_key = tf.squeeze(access.get_key_feature(init_feature, 'init_memory_key'), [1, 2])
        ...
        access_state = access.AccessState(init_memory=init_feature,                     # note here !!!!!!!!!!!!!!!!!!!!!!!!!
                                   memory=memory,
