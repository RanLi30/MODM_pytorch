...
def attention(input, query, scope=None):
    ...
    weighted_sum=tf.reduce_sum(input*score, [1,2])
...
class LSTMmotion(tf.nn.rnn_cell.RNNCell):
    def __init__(self, hidden_size, locations, is_train):
        super(LSTMmotion, self).__init__()
        keep_prob = config.keep_prob if is_train else 1.0
        self._MT = tf.contrib.rnn.LayerNormBasicLSTMCell(hidden_size, layer_norm=True,
    ...
    def __call__(self, inputs, prev_state, scope=None):
        ...
        prev_MT_state = prev_state.MT_state

        #cur_locations = inputs[0]
        #prev_locations = inputs[1]

        MT_output, MT_state = self._MT(inputs, prev_MT_state, scope)
